...
import '../pages/selected/division.dart';
import '../pages/selected/group.dart';
import '../pages/selected/location.dart';
import '../styles/colours.dart';
import '../styles/texts.dart';
import 'interval.dart';

class Selected extends StatefulWidget with PageState {
  const Selected({
    Key? key,
  }) : super(key: key);
...
class _SelectedState extends State<Selected>
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      cloudCtx.section.type != "location"
                          ? _section(
                              cloudCtx, themes, textStyling, theme, width)
  ...
  Widget _section(CloudCtx cloudCtx, AppThemes themes, TextStyling textStyling,
      ThemeVar theme, double width) {
    List<Widget> list = [];
    if (cloudCtx.section.type == "region") {
      List<DivisionModel> divisions = cloudCtx.divisions
          .where((ele) => ele.regionID == cloudCtx.section.areaID)
          .toList();
    ...
    } else if (cloudCtx.section.type == "division") {
      List<GroupModel> groups = cloudCtx.groups
          .where((ele) => ele.divisionID == cloudCtx.section.areaID)
          .toList();
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 5),
                Text(
                  "${cloudCtx.regions.where((ele) => ele.regionID == group.regionID).first.name} Region",
                  style: textStyling.medium14,
                  textAlign: TextAlign.center,
                ),
              ],
    ...
    } else if (cloudCtx.section.type == "group") {
      List<LocalModel> locations = cloudCtx.locations
          .where((ele) => ele.groupID == cloudCtx.section.areaID)
          .toList();
      for (LocalModel location in locations) {
        list.add(
          GestureDetector(
            onTap: () {
              cloudCtx.addSelected(SelectedSection(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 5),
                  Text(
                    "${cloudCtx.divisions.where((ele) => ele.divisionID == location.divisionID).first.name} Division",
                    style: textStyling.medium14,
                    textAlign: TextAlign.center,
                  ),
                  const SizedBox(height: 2),
                  Text(
                    "${cloudCtx.regions.where((ele) => ele.regionID == location.regionID).first.name} Region",
                    style: textStyling.medium14,
                    textAlign: TextAlign.center,
                  ),
                ],
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text(
                _whichSection(cloudCtx.section.type!),
                style: textStyling.bold19,
              ),
            ],
          ),
        ),
  ...
  String _whichSection(String type) {
    switch (type) {
      case "region":
        return "Divisions";
      case "division":
